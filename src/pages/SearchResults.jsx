...
Indicate data_freshness (e.g. "Live - just now").`,
      ...
      setDataAge(res.data_freshness || "Live");
    } catch {
      setWebResults([]);
    }
    setWebLoading(false);
  ...
  const handleSearch = (q) => {
    const finalQ = (typeof q === "string" ? q : inputValue).trim();
    setQuery(finalQ);
  ...
  const handleTabSwitch = (tabId) => {
    setActiveTab(tabId);
    if (tabId === "web" && !webResults.length && !webLoading && query) {
          ...
          <div>
            {webLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <h3 className="font-heading font-semibold text-lg mb-2">Live Web Results</h3>
                <p className="text-muted-foreground text-sm mb-6">Search for shoes to see live prices from Nike, Adidas, Zappos, StockX and more.</p>
                {query && (
                  <button
                    onClick={() => fetchWebResults(query)}
