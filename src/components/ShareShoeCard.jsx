...
import { X, Download, Instagram, MessageCircle, Share2, Copy, Check, Link as LinkIcon } from "lucide-react";
...
export default function ShareShoeCard({ shoe, onClose }) {
  ...
  const shareInstagram = () => {
    // Instagram doesn't have a direct web share URL — copy link + open app
    copyLink();
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      // Build a temporary off-screen card to capture
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 pt-3 pb-4 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {discount > 0 && (
                <span className="text-[10px] bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-400 font-semibold px-1.5 py-0.5 rounded-full">
                  -{discount}%
                </span>
              )}
            </div>
        ...
        {/* Safe area bottom */}
        <div className="h-safe-area-inset-bottom pb-2 flex-shrink-0" />
      </motion.div>
    </div>
  );
}